...
def yield_sign_detection(img_in):
    ...
    #img_in = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, kernel)
    ...
    #lines = cv2.HoughLinesP(edges,1,np.pi/180,80)
    lines = cv2.HoughLinesP(edges,1,np.pi*90/180,70)
#    cv2.imshow('Yield lines.jpg',cimg2)
#    cv2.waitKey(0)
    #lines = cv2.HoughLinesP(edges,1,np.pi/180,80,95,20)
...
#    #print len(lines[0])
#    #print lines
#    cv2.imshow('Yield lines.jpg',img)
#    cv2.waitKey(0)
...
#    canny_para2 = 50
    ...
    #lines = cv2.HoughLinesP(edges,1,np.pi/180,80)
    #lines = cv2.HoughLinesP(edges,1,np.pi,5)
    lines = cv2.HoughLinesP(edges,1,np.pi*90/180,10)
#    for x1,y1,x2,y2 in lines[0]:
#        cv2.line(img,(x1,y1),(x2,y2),(255,0,0),2)
...
#    cv2.waitKey(0)
    ...
    if flag == 1:                    
        ...
        center = (int(center_xfinal), int(center_yfinal))
        return center
    else:
        return (0,0)
    #print center
...
def traffic_sign_detection_noisy(img_in):
    ...
    img2 = img_in.copy()
    signs_dict = {}
    img_in = cv2.bilateralFilter(img_in,9,90,90)
    
    radii_range = [10,40]
#    kernel = np.ones((3,3),np.uint8)
#    img_in = cv2.morphologyEx(img_in, cv2.MORPH_CLOSE, kernel)
#    cv2.imshow('morph', img_in)
#    cv2.waitKey(0)
    (center, state) = traffic_light_detection(img_in, radii_range)
    if center[0] > 0 and center[1] > 0 :
        signs_dict.update({'traffic_light': (center, state)})
